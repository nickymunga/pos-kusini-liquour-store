...
class Products extends MY_Controller
{
    ...
    function get_products($store_id = 1) {

        $this->load->library('datatables');
        if (($this->Admin || $this->Barman)) {
            $this->datatables->select($this->db->dbprefix('products').".id as pid, ".$this->db->dbprefix('products').".image as image, ".$this->db->dbprefix('products').".code as code, ".$this->db->dbprefix('products').".name as pname, type, ".$this->db->dbprefix('categories').".name as cname, psq.quantity, tax, tax_method, cost, (CASE WHEN psq.price > 0 THEN psq.price ELSE {$this->db->dbprefix('products')}.price END) as price, (CASE WHEN psq.ws_price > 0 THEN psq.ws_price ELSE {$this->db->dbprefix('products')}.ws_price END) as ws_price, barcode_symbology", FALSE);
        } else {
            $this->datatables->select($this->db->dbprefix('products').".id as pid, ".$this->db->dbprefix('products').".image as image, ".$this->db->dbprefix('products').".code as code, ".$this->db->dbprefix('products').".name as pname, type, ".$this->db->dbprefix('categories').".name as cname, psq.quantity, tax, tax_method, (CASE WHEN psq.price > 0 THEN psq.price ELSE {$this->db->dbprefix('products')}.price END) as price, (CASE WHEN psq.ws_price > 0 THEN psq.ws_price ELSE {$this->db->dbprefix('products')}.ws_price END) as ws_price, barcode_symbology", FALSE);
        }
    ...
    function print_labels() {
        ...
        $html = "";
        foreach ($products as $pr) {
            $html .= '<div class="text-center labels break-after" style="font-size:22px; margin-bottom:15px;">' . $pr->name . '<br>' . '<span class="price" style="padding-top: 5px;">'.$this->Settings->currency_prefix. ' ' . $this->tec->formatMoney($pr->price) . '</span></div>';
        }
        $this->data['links'] = $pagination;
        $this->data['html'] = $html;
    ...
    function add() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
				...
				'ws_price' => $this->input->post('ws_price'),
            ...
            if ($this->Settings->multi_store) {
                ...
                foreach ($stores as $store) {
                    $store_quantities[] = array(
						...
						'ws_price' => $this->input->post('ws_price'.$store->id)
                        );
                }
            } else {
                $store_quantities[] = array(
                    ...
                    'price' => $this->input->post('price'),
					'ws_price' => $this->input->post('ws_price1')
                    );
            }
