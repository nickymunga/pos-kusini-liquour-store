...
class Pos extends MY_Controller {
    ...
    function get_product($code = NULL) {
        ...
        if($product = $this->pos_model->getProductByCode($code)) {
            ...
            $product->unit_ws_price = $product->tax ? ($product->ws_price+(($product->ws_price*$product->tax)/100)) : $product->ws_price;
    ...
    function close_register($user_id = NULL) {
        ...
        $this->form_validation->set_rules('total_cash_submitted', lang("total_cash"), 'trim|required|numeric');
        $this->form_validation->set_rules('total_mpesa_submitted', lang("total_mpesa"), 'trim|required|numeric');
        ...
        if ($this->form_validation->run() == true) {
            if ($this->Admin) {
                ...
                $mpesasales = $this->pos_model->getRegisterMpesaSales($register_open_time, $user_id);
                ...
                $mpesasales = $this->pos_model->getRegisterMpesaSales($register_open_time);
            ...
            $data = array('closed_at' => date('Y-m-d H:i:s'),
                ...
                'total_mpesa' => $mpesasales->total_mpesa,
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['mpesasales'] = $this->pos_model->getRegisterMpesaSales($register_open_time, $user_id);
    ...
    function print_register($re = NULL) {
        ...
        if ($this->session->userdata('register_id')) {
            ...
            $mpesasales = $this->pos_model->getRegisterMpesaSales();
            ...
            $reg_totals = array(
                ...
                (object) array('label' => lang('mpesa_sale'), 'value' => $this->tec->formatMoney($mpesasales->paid ? $mpesasales->paid : '0.00') . ' (' . $this->tec->formatMoney($mpesasales->total ? $mpesasales->total : '0.00') . ')'),
